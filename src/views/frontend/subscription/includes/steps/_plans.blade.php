<div class="cmb_services_wrapper">
	<div class="row checkbox-type">
		<div class="col-12 mb-2">
			<span class="lead text-black font-weight-bold fw-bold">Selecteer het abonnement dat je wil boeken</span>
		</div>
			...
			<div class="py-2 px-3 border rounded row no-gutters g-0">
				<div class="form-check col-md-11">
				  <label class="form-check-label row" for="subscription_plan_check{{ $plan->id }}">
				    ...
				    <span class="col-md-9 font-weight-bold fw-bold">
				    	<span class="cmb_subscription_plan_name">{{ $plan->name }}</span>
				    </span>
				    <span class="font-weight-bold fw-bold col-md-3 text-right text-end">{{ '€ '.number_format($plan->price, 2, ',', '.') }}</span>
				  </label>
				</div>
				<span class="col-md-1 text-right text-end collapsed" type="button" data-toggle="collapse" data-target="#service_description{{ $plan->id }}" aria-expanded="false" aria-controls="service_description{{ $plan->id }}"><i class="fa fa-chevron-down"></i></span>
