			...
			<div class="py-2 px-3 border rounded row no-gutters">
				<div class="form-check col-md-11">
				  <label class="form-check-label row" for="service_check{{ $service->id }}">
				    ...
				    <span class="col-md-9 font-weight-bold"><span class="cmb_services_name">{{ $service->name }}</span> <small class="cmb_services_duration">({{ $service->duration }} min)</small></span>
				    <span class="font-weight-bold col-md-3 text-right">{{ '€ '.number_format($service->price, 2, ',', '.') }}</span>
				  </label>
				</div>
				<span class="col-md-1 text-right collapsed" type="button" data-toggle="collapse" data-target="#service_description{{ $service->id }}" aria-expanded="false" aria-controls="service_description{{ $service->id }}"><i class="fa fa-chevron-down"></i></span>
